...
class TextImageDataset(data.Dataset):
    def __init__(self, texts, file_path, device):
        ...
        self.class_list = [w.strip() for w in open('data/text/class.txt', 'r').readlines()]
    ...
    def __getitem__(self, item):
        ...
        data_file = os.path.join('data/image/data', data_file + '.jpg')
...
def get_iter(text_train, text_valid, text_test,config):
    train_data = TextImageDataset(text_train, 'data/image/train.txt', config.device)
    valid_data = TextImageDataset(text_valid, 'data/image/dev.txt', config.device)
    test_data = TextImageDataset(text_test, 'data/image/test.txt', config.device)
